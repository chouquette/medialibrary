...
bool MetadataAnalyzer::parseAudioFile( IItem& item )
{
    ...
    auto res = sqlite::Tools::withRetries( 3,
            [this, &item, &artists, media, &thumbnail, &album, &genre]() {
        auto t = m_ml->getConn()->newTransaction();
        if ( thumbnail != nullptr )
        {
            ...
            // At this point, the thumbnail was never generated by the media
            // library so we can pass false as isGenerated parameter
            media->setThumbnail( thumbnail->mrl(), thumbnail->origin(), false );
        }
        if ( album == nullptr )
        {
