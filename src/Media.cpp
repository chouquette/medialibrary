...
FilePtr Media::mainFile() const
{
    ...
    assert( !"A media must have an associated main file" );
    return nullptr;
}

bool Media::cache( const std::string& mrl, File::CacheType cacheType, uint64_t fileSize )
{
    ...
    if ( f->type() == IFile::Type::Cache )
        return true;
    auto c = f->cache( mrl, cacheType, fileSize );
    if ( c == nullptr )
        return false;
...
Query<IMedia> Media::searchInSubscription( MediaLibraryPtr ml, const std::string& pattern,
                                           int64_t subId, const QueryParameters* params )
{
    ...
    return make_query<Media, IMedia>( ml, "m.*", std::move( req ), sortRequest( params ), subId,
                                      sqlite::Tools::sanitizePattern( pattern ) ).build();
}
