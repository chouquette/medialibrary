...
CREATE TABLE `MediaArtistRelation` (`media_id`	INTEGER NOT NULL,`artist_id`	INTEGER,FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE,PRIMARY KEY(`media_id`,`artist_id`),FOREIGN KEY(`artist_id`) REFERENCES `Artist`(`id_artist`) ON DELETE CASCADE);
...
CREATE TABLE `LabelFileRelation` (`label_id`	INTEGER,`media_id`	INTEGER,FOREIGN KEY(`label_id`) REFERENCES `Label`(`id_label`) ON DELETE CASCADE,PRIMARY KEY(`label_id`,`media_id`),FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE);
CREATE TABLE `Label` (`id_label`	INTEGER PRIMARY KEY AUTOINCREMENT,`name`	TEXT UNIQUE);
...
CREATE TABLE `ExcludedEntryFolder` (`folder_id`	UNSIGNED INTEGER NOT NULL UNIQUE,FOREIGN KEY(`folder_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE);
CREATE TABLE `Device` (`id_device`	INTEGER PRIMARY KEY AUTOINCREMENT,`uuid`	TEXT UNIQUE COLLATE NOCASE,	`scheme`	TEXT,`is_removable`	BOOLEAN,`is_present`	BOOLEAN,`last_seen`	UNSIGNED INTEGER);
...
INSERT INTO PlaylistMediaRelation VALUES(1,NULL,1,0);
INSERT INTO PlaylistMediaRelation VALUES(1,NULL,1,1);
INSERT INTO PlaylistMediaRelation VALUES(1,NULL,1,2);
INSERT INTO PlaylistMediaRelation VALUES(1,NULL,2,0);
INSERT INTO PlaylistMediaRelation VALUES(2,NULL,2,1);
INSERT INTO PlaylistMediaRelation VALUES(3,NULL,2,2);
COMMIT;
