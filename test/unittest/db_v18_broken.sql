...
CREATE TABLE IF NOT EXISTS `Folder` (`id_folder`	INTEGER PRIMARY KEY AUTOINCREMENT,`path`	TEXT,`name`	TEXT COLLATE NOCASE,`parent_id`	UNSIGNED INTEGER,`is_banned`	BOOLEAN NOT NULL DEFAULT 0,`device_id`	UNSIGNED INTEGER,`is_removable`	BOOLEAN NOT NULL,`nb_audio`	UNSIGNED INTEGER NOT NULL DEFAULT 0,`nb_video`	UNSIGNED INTEGER NOT NULL DEFAULT 0,FOREIGN KEY(`parent_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE,FOREIGN KEY(`device_id`) REFERENCES `Device`(`id_device`) ON DELETE CASCADE,UNIQUE(`path`,`device_id`));
CREATE TABLE IF NOT EXISTS `File` (`id_file`	INTEGER PRIMARY KEY AUTOINCREMENT,`media_id`	UNSIGNED INT DEFAULT NULL,`playlist_id`	UNSIGNED INT DEFAULT NULL,`mrl`	TEXT,`type`	UNSIGNED INTEGER,`last_modification_date`	UNSIGNED INT,`size`	UNSIGNED INT,`folder_id`	UNSIGNED INTEGER,`is_removable`	BOOLEAN NOT NULL,`is_external`	BOOLEAN NOT NULL,`is_network`	BOOLEAN NOT NULL,UNIQUE(`mrl`,`folder_id`),FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE,FOREIGN KEY(`folder_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE,FOREIGN KEY(`playlist_id`) REFERENCES `Playlist`(`id_playlist`) ON DELETE CASCADE);
