...
CREATE TABLE IF NOT EXISTS `Task` (`id_task`	INTEGER PRIMARY KEY AUTOINCREMENT,`step`	INTEGER NOT NULL DEFAULT 0,`retry_count`	INTEGER NOT NULL DEFAULT 0,`type`	INTEGER NOT NULL,`mrl`	TEXT,`file_type`	INTEGER NOT NULL,`file_id`	UNSIGNED INTEGER,`parent_folder_id`	UNSIGNED INTEGER,`link_to_id`	UNSIGNED INTEGER,`link_to_type`	UNSIGNED INTEGER,`link_extra`	UNSIGNED INTEGER,FOREIGN KEY(`file_id`) REFERENCES `File`(`id_file`) ON DELETE CASCADE,UNIQUE(`mrl`,`type`),FOREIGN KEY(`parent_folder_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS `MediaArtistRelation` (`media_id`	INTEGER NOT NULL,`artist_id`	INTEGER,FOREIGN KEY(`artist_id`) REFERENCES `Artist`(`id_artist`) ON DELETE CASCADE,PRIMARY KEY(`media_id`,`artist_id`),FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS `Folder` (`id_folder`	INTEGER PRIMARY KEY AUTOINCREMENT,`path`	TEXT,`name`	TEXT COLLATE NOCASE,`parent_id`	UNSIGNED INTEGER,`is_banned`	BOOLEAN NOT NULL DEFAULT 0,	`device_id`	UNSIGNED INTEGER,`is_removable`	BOOLEAN NOT NULL,`nb_audio`	UNSIGNED INTEGER NOT NULL DEFAULT 0,`nb_video`	UNSIGNED INTEGER NOT NULL DEFAULT 0,UNIQUE(`path`,`device_id`),FOREIGN KEY(`device_id`) REFERENCES `Device`(`id_device`) ON DELETE CASCADE,FOREIGN KEY(`parent_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS `File` (`id_file`	INTEGER PRIMARY KEY AUTOINCREMENT,`media_id`	UNSIGNED INT DEFAULT NULL,`playlist_id`	UNSIGNED INT DEFAULT NULL,`mrl`	TEXT,`type`	UNSIGNED INTEGER,`last_modification_date`	UNSIGNED INT,`size`	UNSIGNED INT,`folder_id`	UNSIGNED INTEGER,	`is_removable`	BOOLEAN NOT NULL,`is_external`	BOOLEAN NOT NULL,`is_network`	BOOLEAN NOT NULL,FOREIGN KEY(`playlist_id`) REFERENCES `Playlist`(`id_playlist`) ON DELETE CASCADE,FOREIGN KEY(`folder_id`) REFERENCES `Folder`(`id_folder`) ON DELETE CASCADE,UNIQUE(`mrl`,`folder_id`),FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE);
...
CREATE TABLE IF NOT EXISTS `AlbumTrack` (`id_track`	INTEGER PRIMARY KEY AUTOINCREMENT,`media_id`	INTEGER UNIQUE,`duration`	INTEGER NOT NULL,`artist_id`	UNSIGNED INTEGER,`genre_id`	INTEGER,`track_number`	UNSIGNED INTEGER,`album_id`	UNSIGNED INTEGER NOT NULL,`disc_number`	UNSIGNED INTEGER,FOREIGN KEY(`artist_id`) REFERENCES `Artist`(`id_artist`) ON DELETE CASCADE,FOREIGN KEY(`media_id`) REFERENCES `Media`(`id_media`) ON DELETE CASCADE,FOREIGN KEY(`album_id`) REFERENCES `Album`(`id_album`) ON DELETE CASCADE,FOREIGN KEY(`genre_id`) REFERENCES `Genre`(`id_genre`));
...
INSERT INTO `Task` (id_task,step,retry_count,type,mrl,file_type,file_id,parent_folder_id,link_to_id,link_to_type,link_extra) VALUES (1,0,0,0,'file:///root/path/movie.mkv',1,1,1,NULL,NULL,NULL);
