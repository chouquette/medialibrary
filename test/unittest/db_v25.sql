...
CREATE TABLE MediaGroup(id_group INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT COLLATE NOCASE,nb_video UNSIGNED INTEGER DEFAULT 0,nb_audio UNSIGNED INTEGER DEFAULT 0,nb_unknown UNSIGNED INTEGER DEFAULT 0,duration INTEGER DEFAULT 0,creation_date INTEGER NOT NULL,last_modification_date INTEGER NOT NULL,user_interacted BOOLEAN,forced_singleton BOOLEAN);
...
CREATE TRIGGER media_group_increment_nb_media AFTER UPDATE OF type, group_id ON Media WHEN new.group_id IS NOT NULL AND (old.type != new.type OR IFNULL(old.group_id, 0) != new.group_id) BEGIN UPDATE MediaGroup SET nb_video = nb_video + (CASE new.type WHEN 1 THEN 1 ELSE 0 END), nb_audio = nb_audio + (CASE new.type WHEN 2 THEN 1 ELSE 0 END), nb_unknown = nb_unknown + (CASE new.type WHEN 0 THEN 1 ELSE 0 END), last_modification_date = strftime('%s') WHERE id_group = new.group_id; END;
CREATE TRIGGER media_group_decrement_nb_media AFTER UPDATE OF type, group_id ON Media WHEN old.group_id IS NOT NULL AND(old.type != new.type OR old.group_id != IFNULL(new.group_id, 0)) BEGIN UPDATE MediaGroup SET nb_video = nb_video - (CASE old.type WHEN 1 THEN 1 ELSE 0 END), nb_audio = nb_audio - (CASE old.type WHEN 2 THEN 1 ELSE 0 END), nb_unknown = nb_unknown - (CASE old.type WHEN 0 THEN 1 ELSE 0 END), last_modification_date = strftime('%s') WHERE id_group = old.group_id; END;
CREATE TRIGGER media_group_decrement_nb_media_on_deletion AFTER DELETE ON Media WHEN old.group_id IS NOT NULL BEGIN UPDATE MediaGroup SET nb_video = nb_video - (CASE old.type WHEN 1 THEN 1 ELSE 0 END), nb_audio = nb_audio - (CASE old.type WHEN 2 THEN 1 ELSE 0 END), nb_unknown = nb_unknown - (CASE old.type WHEN 0 THEN 1 ELSE 0 END), last_modification_date = strftime('%s') WHERE id_group = old.group_id; END;
CREATE TRIGGER media_group_delete_empty_group AFTER UPDATE OF nb_video, nb_audio, nb_unknown ON MediaGroup WHEN new.nb_video = 0 AND new.nb_audio = 0 AND new.nb_unknown = 0 BEGIN DELETE FROM MediaGroup WHERE id_group = new.id_group; END;
...
INSERT INTO AudioTrack VALUES(4,'mpga',128000,44100,2,'','',4);
...
INSERT INTO Task VALUES(1,3,0,0,'file:///home/chouquette/dev/medialibrary/test/samples/samples/music/compilation/track.mp3',1,1,1,0,0,0,'');
